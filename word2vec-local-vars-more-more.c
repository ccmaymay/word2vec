...
unsigned long long next_random_ull(unsigned long long* rng_state) {
  *rng_state = (*rng_state) * (unsigned long long)25214903917 + 11;
  return *rng_state;
}

real next_random_real(unsigned long long* rng_state) {
  unsigned long long r = next_random_ull(rng_state);
  return (r & 0xFFFF) / 65536.;
}

long long draw_negative_sample(const int* table, unsigned long long* rng_state) {
  unsigned long long r = next_random_ull(rng_state);
  long long neg_sample = table[(r >> 16) % table_size];
  return (neg_sample == 0) ? (r % (vocab_size - 1) + 1) : neg_sample;
}
...
void InitNet(real** input_embeddings, real** syn1, real** output_embeddings, struct vocab_word* vocab) {
  ...
  if (hs) {
    ...
    zero_vector(vocab_size * embedding_size, *syn1);
  }
  if (negative>0) {
  ...
  for (long long a = 0; a < vocab_size; a++) for (long long b = 0; b < embedding_size; b++) {
    (*input_embeddings)[a * embedding_size + b] = (next_random_real(&rng_state) - 0.5) / embedding_size;
  }
...
long long read_new_sentence(FILE* fi, const struct vocab_word* vocab,
                            ...
                            long long* sen, unsigned long long* rng_state,
                            char* eof) {
  ...
  while (1) {
    ...
    if (sample > 0) {
      real threshold =
        ...
        (sample * train_words) /
        vocab[word].cn;
      if (next_random_real(rng_state) > threshold)
        continue;
    }
...
void *TrainModelThread(void *raw_data) {
  ...
  real* input_embeddings = data->input_embeddings;
  real* syn1 = data->syn1;
  real* output_embeddings = data->output_embeddings;
  ...
  unsigned long long rng_state = (long long)id;
  ...
  while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (long long a = dyn_window_offset; a < window * 2 + 1 - dyn_window_offset; a++) if (a != window) {
        ...
        for (long long c = 0; c < embedding_size; c++) output_word_gradient[c] += input_embeddings[c + input_word * embedding_size];
        cw++;
      }
      if (cw) {
        ...
        if (negative > 0) for (long long d = 0; d < negative + 1; d++) {
          ...
          if (d == 0) {
            ...
            target_word = draw_negative_sample(table, &rng_state);
            if (target_word == output_word) continue;
            is_output = 0;
          }
          long long l2 = target_word * embedding_size;
          real f = 0;
          for (long long c = 0; c < embedding_size; c++) f += output_word_gradient[c] * output_embeddings[c + l2];
          ...
          for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] += gradient_scale * output_embeddings[c + l2];
          for (long long c = 0; c < embedding_size; c++) output_embeddings[c + l2] += gradient_scale * output_word_gradient[c];
        }
        // hidden -> in
        for (long long a = dyn_window_offset; a < window * 2 + 1 - dyn_window_offset; a++) if (a != window) {
          long long input_word_position = output_word_position - window + a;
          ...
          long long input_word = sen[input_word_position];
          if (input_word == -1) continue;
          for (long long c = 0; c < embedding_size; c++) input_embeddings[c + input_word * embedding_size] += input_word_gradient[c];
        }
      }
      ...
      for (long long a = dyn_window_offset; a < window * 2 + 1 - dyn_window_offset; a++) if (a != window) {
        ...
        if (hs) for (long long d = 0; d < (*vocab)[output_word].codelen; d++) {
          real f = 0;
          long long output_embeddings_offset = (*vocab)[output_word].point[d] * embedding_size;
          // Propagate hidden -> output
          for (long long c = 0; c < embedding_size; c++) f += input_embeddings[c + input_embeddings_offset] * syn1[c + output_embeddings_offset];
          if (f <= -MAX_EXP) continue;
          else if (f >= MAX_EXP) continue;
          else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
          // 'g' is the gradient multiplied by the learning rate
          real gradient_scale = (1 - (*vocab)[output_word].code[d] - f) * alpha;
          // Propagate errors output -> hidden
          for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] += gradient_scale * syn1[c + output_embeddings_offset];
          // Learn weights hidden -> output
          for (long long c = 0; c < embedding_size; c++) syn1[c + output_embeddings_offset] += gradient_scale * input_embeddings[c + input_embeddings_offset];
        }
        // NEGATIVE SAMPLING
        if (negative > 0) for (long long d = 0; d < negative + 1; d++) {
          long long target_word, is_output;
...
void TrainModel(real* expTable, struct vocab_word** vocab, int* vocab_hash) {
  real *input_embeddings, *syn1, *output_embeddings;
  int *table;
  ...
  InitNet(&input_embeddings, &syn1, &output_embeddings, *vocab);

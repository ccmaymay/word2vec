...
long long train_words = 0, word_count_actual = 0, iter = 5, file_size = 0, classes = 0, word_count_effective = 0;
...
void *TrainModelThread(void *id) {
    ...
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        ...
        printf("%cAlpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  ", 13, alpha,
         ...
         word_count_effective / ((real)(now - start + 1) / (real)CLOCKS_PER_SEC * 1000));
        fflush(stdout);
      }
  ...
  printf("\nTrained on %lld effective words\n", word_count_effective);
  fclose(fi);
  free(neu1);
  free(neu1e);
  pthread_exit(NULL);

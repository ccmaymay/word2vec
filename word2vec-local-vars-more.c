...
void InitUnigramTable(struct vocab_word* vocab, int** table) {
...
int SearchVocab(struct vocab_word* vocab, int* vocab_hash, char *word) {
...
int ReadWordIndex(struct vocab_word* vocab, int* vocab_hash, FILE *fin, char *eof) {
...
int AddWordToVocab(struct vocab_word** vocab, int* vocab_hash, char *word) {
...
void InitNet(real** syn0, real** syn1, real** syn1neg, struct vocab_word* vocab) {
  unsigned long long next_random = 1;
  posix_memalign((void **)syn0, 128, (long long)vocab_size * embedding_size * sizeof(real));
  if (*syn0 == NULL) {printf("Memory allocation failed\n"); exit(1);}
  if (hs) {
    posix_memalign((void **)syn1, 128, (long long)vocab_size * embedding_size * sizeof(real));
    if (*syn1 == NULL) {printf("Memory allocation failed\n"); exit(1);}
    for (long long a = 0; a < vocab_size; a++) for (long long b = 0; b < embedding_size; b++)
     (*syn1)[a * embedding_size + b] = 0;
  }
  if (negative>0) {
    posix_memalign((void **)syn1neg, 128, (long long)vocab_size * embedding_size * sizeof(real));
    if (*syn1neg == NULL) {printf("Memory allocation failed\n"); exit(1);}
    for (long long a = 0; a < vocab_size; a++) for (long long b = 0; b < embedding_size; b++)
     (*syn1neg)[a * embedding_size + b] = 0;
  }
  for (long long a = 0; a < vocab_size; a++) for (long long b = 0; b < embedding_size; b++) {
    next_random = next_random * (unsigned long long)25214903917 + 11;
    (*syn0)[a * embedding_size + b] = (((next_random & 0xFFFF) / (real)65536) - 0.5) / embedding_size;
  }
  CreateBinaryTree(vocab);
}

struct TrainModelThread_data {
  long long id;
  real* syn0;
  real* syn1;
  real* syn1neg;
...
void *TrainModelThread(void *raw_data) {
  struct TrainModelThread_data* data = (struct TrainModelThread_data*) raw_data;
  long long id = data->id;
  real* syn0 = data->syn0;
  real* syn1 = data->syn1;
  real* syn1neg = data->syn1neg;
  real* expTable = data->expTable;
  int* table = data->table;
  struct vocab_word** vocab = data->vocab;
  int* vocab_hash = data->vocab_hash;
  ...
  while (1) {
    ...
    for (long long c = 0; c < embedding_size; c++) output_word_gradient[c] = 0;
    for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] = 0;
    next_random = next_random * (unsigned long long)25214903917 + 11;
    long long dyn_window_offset = next_random % window;
    if (cbow) {  //train the cbow architecture
      ...
      for (long long a = dyn_window_offset; a < window * 2 + 1 - dyn_window_offset; a++) if (a != window) {
        ...
        for (long long c = 0; c < embedding_size; c++) output_word_gradient[c] += syn0[c + input_word * embedding_size];
        cw++;
      }
      if (cw) {
        for (long long c = 0; c < embedding_size; c++) output_word_gradient[c] /= cw;
        if (hs) for (long long d = 0; d < (*vocab)[output_word].codelen; d++) {
          real f = 0;
          long long l2 = (*vocab)[output_word].point[d] * embedding_size;
          // Propagate hidden -> output
          for (long long c = 0; c < embedding_size; c++) f += output_word_gradient[c] * syn1[c + l2];
          if (f <= -MAX_EXP) continue;
          else if (f >= MAX_EXP) continue;
          else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
          // 'g' is the gradient multiplied by the learning rate
          real gradient_scale = (1 - (*vocab)[output_word].code[d] - f) * alpha;
          // Propagate errors output -> hidden
          for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] += gradient_scale * syn1[c + l2];
          // Learn weights hidden -> output
          for (long long c = 0; c < embedding_size; c++) syn1[c + l2] += gradient_scale * output_word_gradient[c];
        }
        // NEGATIVE SAMPLING
        if (negative > 0) for (long long d = 0; d < negative + 1; d++) {
          ...
          if (d == 0) {
            ...
            next_random = next_random * (unsigned long long)25214903917 + 11;
            target_word = table[(next_random >> 16) % table_size];
            if (target_word == 0) target_word = next_random % (vocab_size - 1) + 1;
            if (target_word == output_word) continue;
            is_output = 0;
          }
          long long l2 = target_word * embedding_size;
          real f = 0;
          for (long long c = 0; c < embedding_size; c++) f += output_word_gradient[c] * syn1neg[c + l2];
          real gradient_scale = 0;
          if (f > MAX_EXP) gradient_scale = (is_output - 1) * alpha;
          else if (f < -MAX_EXP) gradient_scale = (is_output - 0) * alpha;
          else gradient_scale = (is_output - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
          for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] += gradient_scale * syn1neg[c + l2];
          for (long long c = 0; c < embedding_size; c++) syn1neg[c + l2] += gradient_scale * output_word_gradient[c];
        }
        // hidden -> in
        for (long long a = dyn_window_offset; a < window * 2 + 1 - dyn_window_offset; a++) if (a != window) {
          long long input_word_position = output_word_position - window + a;
          ...
          long long input_word = sen[input_word_position];
          if (input_word == -1) continue;
          for (long long c = 0; c < embedding_size; c++) syn0[c + input_word * embedding_size] += input_word_gradient[c];
        }
      }
      ...
      for (long long a = dyn_window_offset; a < window * 2 + 1 - dyn_window_offset; a++) if (a != window) {
        ...
        long long input_word = sen[input_word_position];
        if (input_word == -1) continue;
        long long l1 = input_word * embedding_size;
        for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] = 0;
        ...
        if (hs) for (long long d = 0; d < (*vocab)[output_word].codelen; d++) {
          ...
          long long l2 = (*vocab)[output_word].point[d] * embedding_size;
          // Propagate hidden -> output
          for (long long c = 0; c < embedding_size; c++) f += syn0[c + l1] * syn1[c + l2];
          if (f <= -MAX_EXP) continue;
          else if (f >= MAX_EXP) continue;
          else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
          // 'g' is the gradient multiplied by the learning rate
          ...
          for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] += gradient_scale * syn1[c + l2];
          // Learn weights hidden -> output
          for (long long c = 0; c < embedding_size; c++) syn1[c + l2] += gradient_scale * syn0[c + l1];
        }
        // NEGATIVE SAMPLING
        if (negative > 0) for (long long d = 0; d < negative + 1; d++) {
          long long target_word, is_output;
          ...
          long long l2 = target_word * embedding_size;
          real f = 0;
          for (long long c = 0; c < embedding_size; c++) f += syn0[c + l1] * syn1neg[c + l2];
          real gradient_scale = 0;
          if (f > MAX_EXP) gradient_scale = (is_output - 1) * alpha;
          else if (f < -MAX_EXP) gradient_scale = (is_output - 0) * alpha;
          else gradient_scale = (is_output - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
          for (long long c = 0; c < embedding_size; c++) input_word_gradient[c] += gradient_scale * syn1neg[c + l2];
          for (long long c = 0; c < embedding_size; c++) syn1neg[c + l2] += gradient_scale * syn0[c + l1];
        }
        // Learn weights input -> hidden
        for (long long c = 0; c < embedding_size; c++) syn0[c + l1] += input_word_gradient[c];
      }
    }
...
void TrainModel(real* expTable, struct vocab_word** vocab, int* vocab_hash) {
  ...
  InitNet(&syn0, &syn1, &syn1neg, *vocab);
  if (negative > 0) InitUnigramTable(*vocab, &table);
  start = clock();
  struct TrainModelThread_data data = {
    .id = 0,
    .syn0 = syn0,
    .syn1 = syn1,
    .syn1neg = syn1neg,
  ...
  for (long a = 0; a < num_threads; a++) pthread_create(&pt[a], NULL, TrainModelThread, (void *)&data);
  ...
  if (classes == 0) {
    ...
    for (long a = 0; a < vocab_size; a++) {
      ...
      if (binary) for (long b = 0; b < embedding_size; b++) fwrite(&syn0[a * embedding_size + b], sizeof(real), 1, fo);
      else for (long b = 0; b < embedding_size; b++) fprintf(fo, "%lf ", syn0[a * embedding_size + b]);
      fprintf(fo, "\n");
    }
  } else {
    ...
    for (long a = 0; a < iter; a++) {
      ...
      for (long c = 0; c < vocab_size; c++) {
        for (long d = 0; d < embedding_size; d++) cent[embedding_size * cl[c] + d] += syn0[c * embedding_size + d];
        centcn[cl[c]]++;
      }
      ...
      for (long c = 0; c < vocab_size; c++) {
        ...
        for (long d = 0; d < clcn; d++) {
          x = 0;
          for (long b = 0; b < embedding_size; b++) x += cent[embedding_size * d + b] * syn0[c * embedding_size + b];
          if (x > closev) {
            closev = x;
            closeid = d;
          }

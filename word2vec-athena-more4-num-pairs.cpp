...
long long num_input_output_pairs = 0;
clock_t start;
...
long long draw_negative_sample(const ReservoirSampler<long>& table, const NaiveLanguageModel& language_model) {
  long long neg_sample = table.sample();
  if (neg_sample == 0) {
    uniform_int_distribution<unsigned long long> d(1, language_model.size() - 1);
    return d(get_urng());
  } else {
    return neg_sample;
  }
...
void update_progress_and_learning_rate(const NaiveLanguageModel& language_model) {
  if ((debug_mode > 1)) {
    clock_t now = clock();
  ...
  alpha = starting_alpha * (1 - word_count_actual / (real)(iter * language_model.total() + 1));
...
void TrainModelThread(real* input_embeddings,
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {
      ...
      update_progress_and_learning_rate(language_model);
    ...
    for (long long a = dyn_window_offset; a < window * 2 + 1 - dyn_window_offset; a++) if (a != window) {
      ...
      if (negative > 0) for (long long d = 0; d < negative + 1; d++) {
        ...
        if (d == 0) {
          ...
          target_word = draw_negative_sample(table, language_model);
...
void TrainModel(real* expTable, NaiveLanguageModel& language_model) {
